 ...
 *
 * The NEW C++11 gives rvalue references by which 'move semantics' can be
 * implemented, in particular, a "move assignment operator". In our example,
 * Intvec(33) still needs a constructor/destructor pair for a temporary object
 * but no additional temporary is needed, as the && syntax provides an
 * 'rvalue reference' such that the "move operator switches/swaps the rvalue's
 * internal buffer with its own, arraning it so the rvalue's destructor will
